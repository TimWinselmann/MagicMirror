...
magicMirrorApp.controller("CalendarCtrl", function($scope, $http, $interval, $log, calendar_config) {
    ...
    angular.forEach(calendar_config.ical_urls, function(ical_url, i) {
        $log.debug('Fetching ical file', ical_url);
        $http.get(ical_url.url).then(function(response) {
            parseIcal(response.data, ical_url.color);
        }, function(response) {
            $log.error(response);
        });
    });

    var parseIcal = function(icalData, color) {
                ...
                while (next = expand.next()) {
                    ...
                    if (startDate.withoutTime() == now.withoutTime()) {
                        ...
                        $scope.events.push({
                            ...
                            'wholeDayEvent': event.duration.days >= 1,
                            'color': color,
                        });
